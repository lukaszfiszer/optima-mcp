...
| **2 — MCPB extension** | Bundle with a native config UI: directory-of-backups **or** SQL server URL; managed SQL Server container; scan + fingerprint + background import of every backup that changed; multi-database tool surface; diagnostic logging; `clean`. | An accountant installs a `.mcpb`, points it at a folder of `.bac` files, and gets an honest capability report per company without opening a terminal ([`08`](08-mcpb-extension.md)). |
...
| **Docker missing or not running on the user's machine** | Backup mode dead — and this is now the *primary* mode | Detect early and specifically, distinguishing not-installed from not-running ([`08`](08-mcpb-extension.md) §8.10). Never auto-install. The live-server mode needs no container. ~~Open sub-risk: a GUI-launched client gives the server a minimal `PATH`.~~ **Downgraded** — Claude Desktop extracts a login-shell `PATH` and applies a floor that names OrbStack and Rancher Desktop explicitly ([`08`](08-mcpb-extension.md) §8.4), so `docker` should resolve. The probe ladder still ships for the extraction-failure and Windows cases, but it no longer gates the mode. |
| **Host silently runs us on a system Node instead of its own** | Runtime becomes a property of the user's machine; bugs we cannot reproduce | Declare `compatibility.runtimes.node` as the spec requires, but set the floor to the true minimum the bundled code needs and **keep it at or below the host's built-in** — a floor above it is what sends the host hunting, and its last-resort branch ignores the range entirely ([`08`](08-mcpb-extension.md) §8.9). Constrains the code: no `node:sqlite`, no APIs above the floor. Check the version at startup and print it in the log banner ([`08`](08-mcpb-extension.md) §8.12). |
| **A bug on a user's machine is undebuggable** | Support becomes guesswork; MCPB users have no terminal | Own log file with rotation and levels, a startup banner naming runtime / mode / `docker` / paths, every import decision and tool call logged, path surfaced by `optima_describe_environment` ([`08`](08-mcpb-extension.md) §8.12). Credentials never enter it. |
...
6. ~~**Which Node version does Claude Desktop provide to a bundled server?**~~ **Answered** ([`08`](08-mcpb-extension.md) §8.9, §8.13): it provides Electron's embedded Node via a `utilityProcess`, not a bundled `node` binary, and the exact version is whatever that Electron build carries. Still open in one respect: the number itself, needed to set `compatibility.runtimes.node` (declared as the spec requires, but at a floor the built-in clears — §8.9). Read it from the log banner on the first real install.
...
- **`compatibility.runtimes.node` declared, with a deliberately low floor.** ([`08`](08-mcpb-extension.md) §8.9) The spec requires the field and other MCPB hosts rely on it. The finding constrains its *value*: the host runs `type: "node"` servers in an Electron `utilityProcess`, and a floor its built-in Node doesn't satisfy makes it fall back to a system Node of its choosing. So the floor must stay at or below the built-in — which is a constraint on what the bundled code may use.
- **Logging is a Phase 2 deliverable, built first.** ([`08`](08-mcpb-extension.md) §8.12) An MCPB user has no terminal, and the three things most likely to break — which Node, which `PATH`, what the background import did — are invisible without it.
...
Phase 1 is delivered; **Phase 2 is the MCPB extension** ([`08`](08-mcpb-extension.md) §8.11). Both gating checks that used to head it — what Node the host provides, and whether a bundled server can reach Docker from a GUI-launched client — have been answered from the specs and the shipped Claude Desktop bundle ([`08`](08-mcpb-extension.md) §8.13). Neither reshapes mode B; one changes a packaging decision (declare no `runtimes.node`), the other downgrades a risk. So the build starts at item 0, the logging that makes every later item debuggable on a machine we don't have. Note that phase 2 also makes the second mask-spike sample trivially ingestible: drop a `.bac` in the folder.
