...
| **Host silently runs us on a system Node instead of its own** | Runtime becomes a property of the user's machine; bugs we cannot reproduce | Declare **no** `compatibility.runtimes.node` — a floor the built-in doesn't satisfy makes the host go hunting for a system Node rather than fail ([`08`](08-mcpb-extension.md) §8.9). Check the version ourselves at startup and print the runtime in the log banner ([`08`](08-mcpb-extension.md) §8.12). |
...
6. ~~**Which Node version does Claude Desktop provide to a bundled server?**~~ **Answered** ([`08`](08-mcpb-extension.md) §8.9, §8.13): it provides Electron's embedded Node via a `utilityProcess`, not a bundled `node` binary, and the exact version is whatever that Electron build carries. The actionable half was never the number — it was that declaring a `runtimes.node` floor switches the host to searching for a *system* Node. We declare none and check at startup. The version itself gets confirmed from the log banner on the first real install.
...
- **No `compatibility.runtimes.node`.** ([`08`](08-mcpb-extension.md) §8.9) The host runs `type: "node"` servers in an Electron `utilityProcess`; a declared range the built-in doesn't satisfy makes it fall back to a system Node of its choosing instead of failing. Declaring nothing pins us to the one runtime we can test.
